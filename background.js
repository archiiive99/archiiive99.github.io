/* Liquid Chrome adapted from React Bits, copyright 2026 David Haz.
 * https://github.com/DavidHDev/react-bits/tree/main/src/content/Backgrounds/LiquidChrome
 * License: assets/REACT-BITS-LICENSE. Grayscale, slow flow and cursor deformation. */
(() => {
  ...
  const fragment = `precision highp float;
    ...
    void main(){
      vec2 uv=(vUv*2.-1.)*vec2(max(aspect,1.),max(1./aspect,1.));
      float t=time*.16;
      vec2 mouse=mix(vec2(.5),pointer.xy,pointer.z);
      for(float i=1.;i<10.;i++){
        uv.x+=.3/i*cos(i*3.*uv.y+t+mouse.x*3.14159);
        uv.y+=.3/i*cos(i*3.*uv.x+t+mouse.y*3.14159);
      }
      vec2 diff=vUv-mouse;
      float dist=length(diff);
      float ripple=sin(10.*dist-t*2.)*.03;
      uv+=diff/(dist+.0001)*ripple*exp(-dist*20.)*pointer.z;
